...
Возможны три исхода, и каждый ведёт в свою сторону: 200 — корень был в гейте; 503 `degraded` — падает чтение профиля в middleware; снова 500 — падение вне контроллера, и тогда смотрим `error_id` из глобального обработчика.ошибка с памятью про чаты возникает сразу после логина - думай еще
