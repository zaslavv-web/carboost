...
const HeroMetricsStrip = () => {
  ...
  return (
    <div className="mt-6 grid grid-cols-3 gap-3 sm:gap-4 md:gap-6 max-w-md">
      {items.map((m) => (
        <div key={m.key} className="border-t border-border pt-2.5">
          <CountUp
            ...
            prefix={m.prefix}
            suffix={m.suffix}
            className="block text-2xl sm:text-2xl md:text-3xl font-semibold tabular-nums leading-none text-foreground"
          />
          <div className="mt-1.5 text-[11px] md:text-[11px] text-muted-foreground leading-snug">
