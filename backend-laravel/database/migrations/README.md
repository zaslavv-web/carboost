...
Принцип: **дамп Supabase — источник истины для бизнес-схемы**. Laravel-миграции
...
# 2. Импортируем последний дамп Supabase
...
| Файл | Назначение |
|---|---|
| `0001_..._baseline_supabase_schema.php` | Проверяет, что дамп импортирован (auth.users, public.profiles и т.д.). Падает с понятной ошибкой, если нет. Создаёт extensions `pgcrypto`, `uuid-ossp`. |
...
- Триггеры Supabase (`handle_new_user`, `sync_step_goals_to_personal` и т.д.) — продолжают работать.
- Schema `auth` — Supabase создал, мы только добавили `remember_token` колонку.
...
- `app/Models/User.php` поверх view `public.users` с bcrypt-проверкой паролей Supabase
