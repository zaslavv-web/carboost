...
всего, что фронтенд раньше делал через `supabase.from`, `supabase.rpc` и
`supabase.storage`. Страницы остаются нетронутыми и переключаются точечно
...
| `app/Http/Controllers/Api/StorageController.php` | 7 бакетов 1:1 с Supabase Storage. Public bucket → постоянный URL, private → `temporaryUrl(ttl)`. |
...
| Файл | Drop-in замена |
|------|----------------|
| `db.ts` (`laravelDb`) | `supabase.from(t).select().eq().order().limit().range().single().maybeSingle().insert().update().upsert().delete()` |
| `rpc.ts` (`laravelRpc`) | `supabase.rpc(name, params)` |
| `storage.ts` (`laravelStorage`) | `supabase.storage.from(bucket).upload/createSignedUrl/getPublicUrl/remove` |
| `realtime.ts` (`laravelRealtime`) | `supabase.channel(name).on('postgres_changes', filter, cb).subscribe()` (lazy-loads `laravel-echo` + `pusher-js`) |

Все возвращают тот же `{ data, error }`, что и supabase-js — переключение page-by-page это одно изменение импорта.
...
- `supabase.rpc(` → `laravelRpc(` (и распаковать аргументы из `{params}` в объект)
- `supabase.storage.from(` → `laravelStorage.from(`
...
AWS_DEFAULT_REGION=...
AWS_BUCKET_PREFIX=careertrack-   # bucket = prefix + supabase-name
