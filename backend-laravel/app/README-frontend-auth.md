...
старый legacy `AuthContext` остаётся по-умолчанию, а Laravel-вариант
...
- Массовая замена `legacy.from(...)` на `laravel.get/post(...)` в страницах
  CRUD — это **Phase 10**.
- Realtime (Postgres changes) → Laravel Reverb — **Phase 11**.
- Storage (`legacy.storage`) → Laravel Storage + signed URLs — **Phase 11**.

Bridge-подход означает, что во время миграции страницы могут смешанно
использовать оба бэкенда (auth — Laravel, данные — legacy) без поломок,
